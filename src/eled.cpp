...
// 	painter.rotate(d_angleDeg);
	...
	if(!d_linearGradient && d_gradientEnabled)
	{
	 QRadialGradient	gradient(.5 * width(), .5 * height(), radius, .5 * (width() - radius), .5 * (height() - radius));
          gradient.setColorAt(d_gradientStart, QColor(Qt::white)); //EColor(Elettra::white));
	  if (isEnabled())
		  gradient.setColorAt(d_gradientStop, ledColor);
	  else
                  gradient.setColorAt(d_gradientStop, QColor(Qt::gray));//EColor(Elettra::gray));
          gradient.setColorAt(1, QColor(Qt::black));//EColor(Elettra::black));
