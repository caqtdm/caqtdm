...
static inline bool qwtIsLogarithmic( const QwtThermoMarker *thermo )
{
...
public:
    PrivateData():
        ...
        scalePos( QwtThermoMarker::LeftScale ),
        spacing( 3 ),
        borderWidth( 2 ),
        pipeWidth( 10 ),
        minValue( 0.0 ),
...
/*!
  ...
  \param scaleDraw ScaleDraw object, that has to be created with
                   new and will be deleted in ~QwtThermoMarker or the next
...
/*!
  Recalculate the QwtThermoMarker geometry and layout based on
  the QwtThermoMarker::contentsRect() and the fonts.
...
/*!
   ...
   \sa setScalePosition()
*/
void QwtThermoMarker::setOrientation( Qt::Orientation o, ScalePos s )
{
...
*/
void QwtThermoMarker::setScalePosition( ScalePos scalePos )
{
...
/*!
   ...
   \sa setScalePosition()
*/
QwtThermoMarker::ScalePos QwtThermoMarker::scalePosition() const
{
    ...
    QPainter *painter, const QRect &pipeRect ) const
{
    ...
    else
    {
        ...
        {
            if(thisType == Marker) {
                fillRect.setLeft( tval + 2 );
                fillRect.setRight(tval - 2 );
            } else if(thisType == PipeFromCenter) {
                const float center = d_data->minValue + (d_data->maxValue-d_data->minValue)/2;
                const int cval = qRound( d_data->map.transform(center));
        ...
        {
            if(thisType == Marker) {
                fillRect.setBottom( tval + 2 );
                fillRect.setTop( tval - 2);
            } else if(thisType == PipeFromCenter) {
                const float center = d_data->minValue + (d_data->maxValue-d_data->minValue)/2;
                const int cval = qRound( d_data->map.transform(center));
...
*/
const QBrush& QwtThermoMarker::fillBrush() const
{
...
/*!
  ...
  \warning The alarm threshold has no effect, when
           a color map has been assigned
*/
const QBrush& QwtThermoMarker::alarmBrush() const
{
...
/*!
  ...
  \warning The alarm threshold has no effect, when
           a color map has been assigned
*/
void QwtThermoMarker::setAlarmEnabled( bool tf )
{
