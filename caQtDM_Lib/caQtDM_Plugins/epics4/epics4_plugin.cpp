...
public:
    ...
    virtual ~PVAChannelRequester() {
        if(Epics4Plugin::getDebug()) qDebug() << "~PVAChannelRequester" ;
    }
...
public:
    ...
    virtual ~PVAGetFieldRequester() {
        if(Epics4Plugin::getDebug()) qDebug() << "~PVAGetFieldRequester" ;
    }
...
public:
    ...
    virtual ~PVAChannelGetRequester() {
        if(Epics4Plugin::getDebug()) qDebug() << "~PVAChannelGetRequester" ;
    }
...
public:
    ...
    virtual ~PVAChannelPutRequester() {
        if(Epics4Plugin::getDebug()) qDebug() << "~PVAChannelPutRequester" ;
    }
...
public:
    ...
    virtual ~PVAMonitorRequester() {
        if(Epics4Plugin::getDebug()) qDebug() << "~PVAMonitorRequester" ;
    }
...
PVAChannel::PVAChannel(const string & fullName, const string & mapName,
    ...
    providerN(providerN)
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAChannel::PVAChannel() fullName " << fullName << " " << mapName ;
}

PVAChannel::~PVAChannel()
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAChannel::~PVAChannel()\n";
}

void PVAChannel::destroy()
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAChannel::destroy()\n";
...
{
    Q_UNUSED(channel);
    if(Epics4Plugin::getDebug()) qDebug() << "PVAChannel::channelStateChange\n";
    Lock xx(mutex);
    if(pvaInterfaceList.empty()) return;
    size_t num = pvaInterfaceList.size();
    bool value = (connectionState==Channel::CONNECTED ? true :  false);
    if(Epics4Plugin::getDebug()) {
        qDebug() << "PVAChannel::channelStateChange isConnected " << ( value ? "true" : "false")
             << " fullName " << getFullName()
             << " num " << num;
    }
...
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAChannel::connect " + channelName +"\n";

    if(!providerN) {
        requester->message(channelName + " provider " + providerName + " not registered",errorMessage);
    }
    if(Epics4Plugin::getDebug()) qDebug() << "PVAChannel::provider=" + providerN->getProviderName()+ "\n";
...
{
    if(Epics4Plugin::getDebug()) {
        qDebug() << "PVAChannel::addInterface"
             << " fullName " << getFullName()
             << " channel->isConnected())  " << ( channel->isConnected() ? "true" : "false");
...
bool PVAChannel::removeInterface(const PVAInterfacePtr & pvaInterface)
{
    ...
    size_t num = pvaInterfaceList.size();
    if(Epics4Plugin::getDebug()) {
        qDebug() << "PVAChannel::removeInterface"
             << " fullName " << getFullName()
             << " num " << num;
    }
    for(size_t ind = 0; ind<num; ++ind) {
    ...
    if(Epics4Plugin::getDebug()) {
        qDebug() << "after remove num " << pvaInterfaceList.size();
    }
...
PVAInterface::PVAInterface(
    ...
    : pvaChannel(pvaChannel),
      ...
      convert(getConvert())
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::PVAInterface()\n";
}
...
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::destroy calling pvaChannelGet->destroy\n";
    if(pvaChannelGet) pvaChannelGet->destroy();
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::destroy calling pvaChannelPut->destroy\n";
    if(pvaChannelPut) pvaChannelPut->destroy();
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::destroy calling pvaChannel.reset\n";
    pvaChannel.reset();
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::destroy return\n";
}
...
{
    Q_UNUSED(structure);
    if(Epics4Plugin::getDebug()) qDebug() << " PVAInterface::monitorConnect\n";
    if(status.isOK()) {
        Lock lock(mutex);
        if(!monitorStarted) {
            monitor->start();
...
{
    Q_UNUSED(monitor);
    qDebug() << "PVAInterface::unlisten\n";
    unlistenCalled = true;
}

void PVAInterface::monitorEvent(MonitorPtr const & monitor)
{
    if(Epics4Plugin::getDebug()) qDebug() << " PVAInterface::monitorEvent\n";
    while(true) {
        ...
        ChannelPut::shared_pointer const & channelPut)
{
    ...
    Lock lock(mutex);
    putFinished = true;
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::putDone set putFinished = true\n";
}
...
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::reconnect() not implemented\n";
    return false;
}

int PVAInterface::disconnect()
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::disconnect()  not implemented\n";
    return false;
}

int PVAInterface::terminateIO()
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::terminateIO()  not implemented\n";
    return false;
}
...
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::gotInterface()\n";
...
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::createMonitor()\n";
    try {
...
{
    if(Epics4Plugin::getDebug()) qDebug() << "PVAInterface::gotMonitor()\n";
}
...
{
    if(Epics4Plugin::getDebug()) qDebug() << "Epics4Plugin::Epics4Plugin\n";
...
int Epics4Plugin::pvAddMonitor(int index, knobData *kData, int rate, int skip)
{
    ...
    if(it != pvaChannelMap.end()) {
        ...
        if(pvaChannel) {
            foundit = true;
            if(Epics4Plugin::getDebug()) qDebug() << "found it\n";
        } else {
            pvaChannelMap.erase(it);
            if(Epics4Plugin::getDebug()) qDebug() << "erase it\n";
        }
    }*/
    foundit = false;
    if(!foundit) {
        ...
        if(Epics4Plugin::getDebug())qDebug() << "created new and called connect\n";
    }
    ...
    if(Epics4Plugin::getDebug()) qDebug() << "calling addInterface\n";
...
{
    if(Epics4Plugin::getDebug()) qDebug()  << "Epics4Plugin:pvGetDescription\n";
    map<std::string, int>::iterator it = pvMap.begin();
    while (it != pvMap.end()) {
        string mapname = it->first;
        if (mapname.find(pv) != string::npos) {
...
int Epics4Plugin::pvClearEvent(void * ptr) {
    if(Epics4Plugin::getDebug()) qDebug()  << "Epics4Plugin:pvClearEvent\n";
...
int Epics4Plugin::pvAddEvent(void * ptr) {
    if(Epics4Plugin::getDebug()) qDebug()  << "Epics4Plugin:pvAddEvent\n";
    PVAInterfaceGlue *pvaInterfaceGlue  = static_cast<PVAInterfaceGlue *>(ptr);
    PVAInterfacePtr pvaInterface = pvaInterfaceGlue->getPVAInterface();
    if(!pvaInterface) throw std::runtime_error("Epics4Plugin::pvSetWave pvaInterface is null");
...
int Epics4Plugin::FlushIO() {
    //if(Epics4Plugin::getDebug()) qDebug() << "Epics4Plugin:FlushIO\n";
    ca_flush_io();
    return true;
}
...
int Epics4Plugin::TerminateIO() {
    Epics4Plugin::setDebug(true);
    if(Epics4Plugin::getDebug()) qDebug() << "Epics4Plugin::TerminateIO calling ClientFactory::stop();\n";
    ClientFactory::stop();
    if(Epics4Plugin::getDebug()) qDebug() << "Epics4Plugin::TerminateIO calling CAClientFactory::stop();\n";
    CAClientFactory::stop();
    if(Epics4Plugin::getDebug()) qDebug() << "Epics4Plugin::TerminateIO calling epics4_callbackThread->stop();\n";
    epics4_callbackThread->stop();
    if(Epics4Plugin::getDebug()) qDebug() << "Epics4Plugin::TerminateIO returning true;\n";
