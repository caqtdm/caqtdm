...
public slots:
    ...
    void getFromArchive(QWidget *w, indexes indexNew,  QString index_name, MessageWindow * messageWindow) {
        ...
        qDebug() << "ArchiveHTTP_plugin.h:117 " << "fromArchive pointer=" << fromArchive << indexNew.timeAxis;
        ...
        if (fromArchive->is_Redirected()){
          ...
          qDebug() << "ArchiveHTTP_plugin.h:131 " << "archiv PV"<<indexNew.pv;
        ...
        qDebug() << "ArchiveHTTP_plugin.h:160 " << QTime::currentTime().toString() << "number of values received" << nbVal << fromArchive << "for" << key;
