...
def main():
	...
	dll1 = ctypes.CDLL(path + "/libedlParser.dylib", mode=ctypes.RTLD_GLOBAL)
	dll2 = ctypes.CDLL(path + "/libadlParser.dylib", mode=ctypes.RTLD_GLOBAL)
	dll3 = ctypes.CDLL(path + "/libqtcontrols.dylib", mode=ctypes.RTLD_GLOBAL)
	dll4 = ctypes.CDLL(path + "/controlsystems/libepics3_plugin.dylib", mode=ctypes.RTLD_GLOBAL)
	dll6 = ctypes.CDLL(path + "/controlsystems/libdemo_plugin.dylib", mode=ctypes.RTLD_GLOBAL)
	dll7 = ctypes.CDLL(path + "/controlsystems/libarchiveSF_plugin.dylib", mode=ctypes.RTLD_GLOBAL)
	dll8 = ctypes.CDLL(path + "/controlsystems/libenvironment_plugin.dylib", mode=ctypes.RTLD_GLOBAL)
	global dll9 
	dll9 = ctypes.CDLL("libcaQtDM_Lib.dylib")
